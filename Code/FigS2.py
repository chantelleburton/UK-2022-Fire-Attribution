...
for member in members:
    ...
    FWI_SIM[member] = fwi_sim
    FWI_OBS[member] = fwi_obs
...
fwi_sim_ENSEMBLE = [FWI_SIM['aojaa'],FWI_SIM['aojab'],FWI_SIM['aojac'],FWI_SIM['aojad'],FWI_SIM['aojae'],FWI_SIM['aojaf'],FWI_SIM['aojag'],FWI_SIM['aojah'],FWI_SIM['aojai'],FWI_SIM['aojaj'],FWI_SIM['dlrja'],FWI_SIM['dlrjb'],FWI_SIM['dlrjc'],FWI_SIM['dlrjd'],FWI_SIM['dlrje']]

fwi_obs_ENSEMBLE = [FWI_OBS['aojaa'],FWI_OBS['aojab'],FWI_OBS['aojac'],FWI_OBS['aojad'],FWI_OBS['aojae'],FWI_OBS['aojaf'],FWI_OBS['aojag'],FWI_OBS['aojah'],FWI_OBS['aojai'],FWI_OBS['aojaj'],FWI_OBS['dlrja'],FWI_OBS['dlrjb'],FWI_OBS['dlrjc'],FWI_OBS['dlrjd'],FWI_OBS['dlrje']]


###Inverse of log transform here###
fwi_detrended_sim_ENSEMBLE = np.log(np.exp(fwi_detrended_sim_ENSEMBLE)+1)
fwi_sim_ENSEMBLE = np.log(np.exp(fwi_sim_ENSEMBLE)+1)
fwi_obs_ENSEMBLE = np.log(np.exp(fwi_obs_ENSEMBLE)+1)
...
#plt.subplot(2,2,3)
plt.plot(years, np.mean(fwi_obs_ENSEMBLE, axis=0), label='ERA5', color='blue')
plt.plot(years, np.mean(fwi_sim_ENSEMBLE, axis=0), label='HadGEM3', color='red')
plt.plot(years, np.mean(fwi_detrended_sim_ENSEMBLE, axis=0), label='Detrended & Shifted FWI', color='purple')

plt.fill_between(years, np.max(fwi_obs_ENSEMBLE, axis=0), np.min(fwi_obs_ENSEMBLE, axis=0),  color='blue', alpha=0.5)
plt.fill_between(years, np.max(fwi_sim_ENSEMBLE, axis=0), np.min(fwi_sim_ENSEMBLE, axis=0),  color='red', alpha=0.5)
plt.fill_between(years, np.max(fwi_detrended_sim_ENSEMBLE, axis=0), np.min(fwi_detrended_sim_ENSEMBLE, axis=0),  color='purple', alpha=0.5)
